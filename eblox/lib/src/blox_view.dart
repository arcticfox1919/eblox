...
class BloxView<T extends Blox, S extends BloxAsyncState> extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    bloxBuilder = BloxBuilder<T, S>(builder: (state) {
      if (_checkFunction(onLoading) &&
          (bloxBuilder.currentState as BloxAsyncState).status ==
              BloxStatus.loading) {
        return onLoading!();
      } else if (state is BloxAsyncState) {
